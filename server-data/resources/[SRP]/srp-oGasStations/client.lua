...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    local finished = exports["srp-taskbar"]:taskBar(timer,"Refueling")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsVehicleEngineOn(veh) then          
            ...
            if not exports["srp-inventory"]:hasEnoughOfItem("watch",1,false) then
                compass_on = false
            end
        end
    end
...
AddEventHandler('carHud:compass', function(table)
    if exports["srp-inventory"]:hasEnoughOfItem("watch",1,false) then
...
RegisterNetEvent("IHATETHIS")
AddEventHandler("IHATETHIS", function()
