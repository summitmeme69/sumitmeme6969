...
function closeGui()
    ...
    ClearPedTasks(PlayerPedId())
    TriggerEvent("srp-taskbar:disablecontrols", false)
end
...
AddEventHandler('coffee:drink2', function()
    if coffeetimer > 0 then
        coffeetimer = 2000
        TriggerEvent("Evidence:StateSet",27,2000)
        return
    else
        TriggerEvent("Evidence:StateSet",27,2000)
        coffeetimer = 2000
...
function taskBar(length,name,runCheck,keepWeapon,vehicle,vehCheck)
    ...
    TriggerEvent("srp-taskbar:disablecontrols", true)
...
RegisterNetEvent('srp-taskbar:disablecontrols')
AddEventHandler('srp-taskbar:disablecontrols', function(status)
    pDisabled = status
    while pDisabled == true do
        Citizen.Wait(0) 
        DisableControlAction(0, 311, true) -- Inv key
    end
end)