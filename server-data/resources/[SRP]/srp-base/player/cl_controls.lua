...
SRP.Controls.secondaryBinds = {}

SRP.Controls.Distcheck = {}
SRP.Controls.Distcheck.use = {
	-- create fake id
...
SRP.Controls.Distcheck.useSecondary = {
...
SRP.Controls.Distcheck.useThird = {
	-- hospital garage
...
function SRP.DataControls.encodeSetBindTable(self, bindTable)
	...
	SRP.DataControls.setBindTable(controlTable,true)
end

function SRP.DataControls.toUpper(table)
...
function SRP.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		SRP.Controls.Current  = SRP.DataControls.toUpper(SRP.Controls.Default) 
		SRP.DataControls.setSecondaryBindTable(SRP.Controls.Current)
		TriggerServerEvent('srp-base:sv:player_control_set',SRP.Controls.Current)
		SRP.DataControls.checkForMissing()
	else
		if shouldSend then 
			SRP.Controls.Current = SRP.DataControls.toUpper(controlTable)
			SRP.DataControls.setSecondaryBindTable(SRP.Controls.Current)
			TriggerServerEvent('srp-base:sv:player_control_set',SRP.Controls.Current)
			SRP.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",SRP.DataControls.getTableInKeyNumbers())
end

function SRP.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#SRP.Controls.secondaryBinds  do
				if SRP.Controls.secondaryBinds[i] and SRP.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				SRP.Controls.secondaryBinds[i] = {}
				SRP.Controls.secondaryBinds[i][1] = control1
				SRP.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function SRP.DataControls.getTableInKeyNumbers()
...
function SRP.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(SRP.Controls.Current) do
		if k == bindID then
			found = v
...
function SRP.DataControls.distanceCall(distcheckName)
	local distanceTable = SRP.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if SRP.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-SRP.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			SRP.Controls.EventHolder[key] = GetGameTimer();
...
function SRP.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(SRP.Controls.Default) do
		if SRP.Controls.Current[j] == nil then
			isMissing = true
			SRP.Controls.Current[j] = h
		end
	end
...
function SRP.DataControls.validEvent(event,id,eventID)
	local v = SRP.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if SRP.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-SRP.Controls.EventHolder[eventKey])
	end

	if SRP.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			SRP.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if SRP.Controls.Current["tokoptt"] then
			for i=0,#SRP.Controls.events["general"] do
				local v = SRP.Controls.events["general"][i]
				local keyString = SRP.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#SRP.Controls.secondaryBinds do
							if IsControlPressed(1,SRP.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,SRP.Controls.secondaryBinds[i][1]) then
								isConflicted = SRP.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								SRP.DataControls.validEvent(eventBind,idEvent,i)
							else
								SRP.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#SRP.Controls.secondaryBinds do
									if IsControlPressed(1,SRP.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,SRP.Controls.secondaryBinds[i][2]) then
										isConflicted = SRP.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if SRP.Controls.Current["tokoptt"] then
			for i=0,#SRP.Controls.events["general-dist"] do
				local v = SRP.Controls.events["general-dist"][i]
				local key = Keys[SRP.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					SRP.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("srp-base:cl:player_control")
AddEventHandler("srp-base:cl:player_control", function(controlTable)
	SRP.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('srp-base:setcontrols')
AddEventHandler('srp-base:setcontrols', function()
	TriggerServerEvent('srp-base:sv:player_controls')
end)
