const mdtApp = new Vue({
    ...
    methods: {
        ...
        closeMDT() {
            $.post('http://srp-mdt/close', JSON.stringify({}));
        },
        getClass(element) {
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://srp-mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://srp-mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://srp-mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://srp-mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://srp-mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://srp-mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        evFile(currentid) {
			$.post('http://srp-mdt/evFile', JSON.stringify({
            id: currentid,
            }));
		},
        ToggleReportEdit() {
        ...
        DeleteSelectedReport() {
            $.post('http://srp-mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://srp-mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        //         $.post('http://srp-mdt/vehicleSearch', JSON.stringify({
        ...
        //     $.post('http://srp-mdt/getVehicle', JSON.stringify({
        //         vehicle: result
        //     }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://srp-mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://srp-mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://srp-mdt/getReport', JSON.stringify({
                id: id
            }));
        ...
        //     $.post('http://srp-mdt/saveVehicleChanges', JSON.stringify({
