...
function DropItemPed()
    ...
    elseif chance == 3 then
        ...
        TriggerServerEvent('mission:finished', math.random(30))
    end
...
function shutoffenginesearch()
    ...
    if not fuckingRETARDED then
       ...
       fuckingRETARDED = true
        ...
        local finished = exports["srp-taskbar"]:taskBar(5000,"Searching")
        Citizen.Wait(100)
        ...
        if luck == 69 and finished == 100 then
          local finished = exports["srp-taskbar"]:taskBar(2000,"Found and Using Keys")
        ...
        local finished = exports["srp-taskbar"]:taskBar(5000,"Searching Backseat")
        if luck > 75 and finished then
          if not IsPedInAnyVehicle(PlayerPedId(), false) then
            fuckingRETARDED = false
            return
          end
          local finished = exports["srp-taskbar"]:taskBar(2000,"Found Content")
          DropItemPed()
        end
...
function shutoffenginehotwire()
    ...
    if not fuckingRETARDED then
        ...
        local finished = exports["srp-taskbar"]:taskBar(carTimer,"Attempting Hotwire")
        Citizen.Wait(100)
          ...
          if not hasKey(plate1) then
            ...
            if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
              ...
              if IsEntityDead(pedDriver) then
                ...
                local finished = exports["srp-taskbar"]:taskBar(3000,"Taking Keys",false)
                ...
                if pedOwner == PlayerId() then
                    ...
                    TriggerServerEvent('srp:peds:decor', GetPlayerServerId(pedOwner), PedToNet(pedDriver))
                end
                ...
                if GetEntityModel(curveh) ~= `taxi` then
                  ...
                  if math.random(100) > 95 then
                      ...
                      local finished = exports["srp-taskbar"]:taskBar(3000,"Taking Keys")
                      if finished == 100 then
                       -- SetEntityAsMissionEntity(curveh,false,true)
                        TriggerEvent("keys:addNew",curveh,plate1)
                      else
