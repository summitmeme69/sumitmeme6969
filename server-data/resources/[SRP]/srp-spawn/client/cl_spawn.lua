...
function Login.SetCharacterLoginCam()
	...
	local camCoords = {-3968.05, 2016.41, 502.7 }
...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["srp-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		local events = exports["srp-base"]:getModule("Events")
		events:Trigger("srp-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
	...
	if data.action == "newCharacter" then
		...
		events:Trigger("srp-base:createCharacter", chardata, function(created)
            if not created then
...
RegisterNetEvent("srp-base:spawnInitialized")
AddEventHandler("srp-base:spawnInitialized", function()
	TriggerServerEvent("commands:reset:login")
	TriggerServerEvent("srp-base:clearStates")
	Login.nativeStart()
end)

RegisterNetEvent("srp-base:relog")
AddEventHandler("srp-base:relog", function(type)
	TriggerServerEvent("commands:reset:login")
	TriggerServerEvent("srp-base:clearStates")
	if type == "motel" then
		Login.nativeStart()
