...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("srp-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with srp-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("srp-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["srp-base"]:getModule("Events")
	events:Trigger("srp-base:selectCharacter", data.actionData, function(returnData)
...
RegisterNetEvent("srp-spawn:finishedClothing")
AddEventHandler("srp-spawn:finishedClothing", function(endType)
