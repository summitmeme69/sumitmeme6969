local stashes = {}
local instash = 0
RegisterNetEvent('srp-stash:setInitialState')
AddEventHandler('srp-stash:setInitialState', function(stashesp)
    stashes = stashesp
    
end)

CreateThread(function()
    TriggerServerEvent("srp-stash:fetchInitialState")
end)
...
RegisterNetEvent('srp-stash:getnearstash')
AddEventHandler('srp-stash:getnearstash', function()
    local closestDoorDistance, closestDoorId = 9999.9, -1
    local currentPos = GetEntityCoords(PlayerPedId())
    for id, handle in pairs(stashes) do
        local currentDoorDistance = #(stashes[id].StashEntry - currentPos)
...
RegisterNUICallback('complete', function(data, cb)
    CloseGui();
    cb('ok');
    TriggerEvent('srp-stash:getnearstash')
end)
