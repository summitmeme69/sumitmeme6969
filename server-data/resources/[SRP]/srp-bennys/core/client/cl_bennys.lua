...
local hayesautos = vector3(-1423.4973144531, -450.17852783203, 35.909660339355)

local civrepairbennys = vector3(-223.74258422852, -1329.9006347656, 30.513158798218) -- Repair shop for civs in bennys
...
function AttemptPurchase(type, upgradeLevel)
    ...
    if nearTuner or nearAutoExotics or nearHayes or nearPaleto then
        cheap = true
    end
    ...
    TriggerServerEvent("srp-bennys:attemptPurchase",cheap, type, upgradeLevel)
...
AddEventHandler('event:control:bennys', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        if useID == 1 and not isPlyInBennys and exports["isPed"]:isPed("myJob") == "paleto_mech" then -- Bennys
            bennyHeading = 44.22135375977
            enterLocation(billyLocationPaleto)
        elseif useID == 2 and not isPlyInBennys and exports["isPed"]:isPed("myJob") == "tuner_shop" then
            bennyHeading = 265.4372253418
            enterLocation(bennyLocationTuner)
        elseif useID == 3 and not isPlyInBennys and exports["isPed"]:isPed("myJob") == "auto_bodies" then
            bennyHeading = 206.49089050293
            enterLocation(hayesautos)
        elseif useID == 4 and not isPlyInBennys and exports["isPed"]:isPed("myJob") == "auto_exotics" then
...
Citizen.CreateThread(function()
    while true do 
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            nearPaleto = isNear(plyPos, billyLocationPaleto, 5)
            nearAutoExotics = isNear(plyPos, AutoExoticsPlace, 5)
            if nearTuner or nearPaleto or nearHayes or nearAutoExotics then
                local job = exports["isPed"]:isPed("myJob")
 
                -- Paleto
                if nearPaleto and job == "paleto_mech" then
                    if not isPlyInBennys then
                        Draw3DText(billyLocationPaleto.x, billyLocationPaleto.y, billyLocationPaleto.z + 0.5, "[Press ~p~E~w~ - Enter Paleto Performance]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                ...
                -- Auto Bodies
                if nearHayes and job == "auto_bodies" then
                    if not isPlyInBennys then
                        Draw3DText(hayesautos.x, hayesautos.y, hayesautos.z + 0.5, "[Press ~p~E~w~ - Enter Hayes Auto]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustReleased(1, 38) then
...
--#[Event Handlers]#--
RegisterNetEvent("srp-bennys:purchaseSuccessful")
AddEventHandler("srp-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("srp-bennys:purchaseFailed")
AddEventHandler("srp-bennys:purchaseFailed", function()
...
Citizen.CreateThread(function()
    exports["srp-polyzone"]:AddBoxZone("Civ_repair", vector3(-222.73, -1329.78, 30.89), 9.0, 5, {
        name="Civ_repair",
        heading=90,
        --debugPoly=true,
        minZ=29.89,
...
AddEventHandler('srp-polyzone:enter', function(name)
    ...
    if name == "Civ_repair" then
        if vehicle ~= 0 then
            NearCiv = true
            CivRepairFunction()
            TriggerEvent('srp-ui:ShowUI', 'show', ("[E] %s"):format("Repair ($450)"))
        end
    end
end)
        ...
        while NearCiv do
            ...
            if IsControlJustReleased(0, 38) then
                if car ~= 0 then
                    ...
                    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10, 'wrench', 0.2)
                    SetEntityHeading(PlayerPedId(), 271.98712158203)
                    local finished = exports["srp-taskbar"]:taskBar(24000,"Repairing")
                    if finished == 100 then
                        ...
                        TriggerServerEvent("srp-bennys:repairciv")
                    else
                        FreezeEntityPosition(car, false)
                    end
                end
