...
function removePlayerFromCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        TriggerClientEvent('srp-voice:removePlayerFromCall', player, source)
    end
...
function addPlayerToCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        ...
        if player ~= source then
            TriggerClientEvent('srp-voice:addPlayerToCall', player, source)
        end
    end
    callData[callChannel][source] = false
    voiceData[source] = voiceData[source] or defaultTable(source)
    voiceData[source].call = callChannel
    TriggerClientEvent('srp-voice:syncCallData', source, callData[callChannel])
end
...
function setPlayerCall(source, callChannel)
    ...
    if GetInvokingResource() then
        ...
        -- changed
        TriggerClientEvent('srp-voice:clSetPlayerCall', source, callChannel)
    end
    voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('srp-voice:setPlayerCall', function(callChannel)
    ...
    if callTbl then
        ...
        for player, _ in pairs(callTbl) do
            if player ~= source then
                ...
                TriggerClientEvent('srp-voice:setTalkingOnCall', player, source, talking)
            end
        end
    else
...
RegisterNetEvent('srp-voice:setTalkingOnCall', setTalkingOnCall)