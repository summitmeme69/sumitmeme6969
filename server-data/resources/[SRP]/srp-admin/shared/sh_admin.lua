...
SRP._Admin.CurAdmins = {}

function SRP.Admin.Log(self, log, user)
...
function SRP.Admin.ChatPrint(self, msg, target)

end

function SRP.Admin.RankExists(self, rank)
    rank = tostring(rank)
...
function SRP.Admin.CommandExists(self, cmd)
    cmd = tostring(cmd)
...
function SRP.Admin.RankInheritsCommand(self, rank, cmd)
...
function SRP.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return exports["srp-base"]:getModule("LocalPlayer"):getVar("rank")
    else
...
function SRP.Admin.RankHasCommand(self, rank, cmd)
...
function SRP.Admin.GetRanks(self)
    return SRP._Admin.Ranks
end

function SRP.Admin.GetRankData(self, rank)
    if not rank then return false end
...
function SRP.Admin.GetCommands(self)
    return SRP._Admin.Commands
end

function SRP.Admin.GetCategories(self)
    local tmp = {}
...
function SRP.Admin.GetCommandData(self, cmd)
    if not cmd then return false end
...
function SRP.Admin.IsAdmin(self, user)
    if not user then return false end
...
function SRP.Admin.IsSuperAdmin(self, user)
    if not IsDuplicityVersion() then
...
function SRP.Admin.IsValidUser(self, user)
    if not user then return false end
    ...
    local util = exports["srp-base"]:getModule("Util")
    if not util:IsSteamId(steamid) then return false end

    return true
end

function SRP.Admin.CanTarget(self, user, target)
...
function SRP.Admin.CanTargetRank(self, user, rank)
...
function SRP.Admin.AddCommand(self, cmd)
    local tmp = {}
    ...
    if IsDuplicityVersion() then
        ...
        tmp.runcommand = function(args) args.command = tmp.command SRP.Admin:RunCommand(args) end
...
function SRP.Admin.AddRank(self, rank, data)
    local tmp = {}
...
function SRP.Admin.GetBanTimeFromString(self, time)
...
function SRP.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        if not player then else SRP._Admin.Players[src].status = status end

        for k,v in pairs(SRP._Admin.CurAdmins) do
            TriggerClientEvent("srp-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("srp-admin:setStatus", status)
end

function SRP.Admin.DumpCurrentPlayers(self)
    TriggerServerEvent("admin:dumpCurrentPlayers",SRP._Admin.Players,SRP._Admin.DiscPlayers)
end

AddEventHandler("srp-base:exportsReady", function()
    exports["srp-base"]:addModule("Admin", SRP.Admin)
    exports["srp-base"]:addModule("_Admin", SRP._Admin)
end)