        ...
        if (action !== undefined) {
            switch (action) {
                ...
                case "newPostSubmit":
                    ...
                    break;
                case "group-manage":
                    $.post('http://phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data'), GroupID2: $(this).data('action-data-name'), GroupID3: $(this).data('action-data-rank') }));
                    break;
                case "btnTaskGang":
    ...
    window.addEventListener('message', function(event) {
        ...
        switch (item.openSection) {
            ...
            case "groups":
                ...
                break; // me and fizz has the same version
            case "addTasks":
            ...
            case "callState":
                ...
                break;
            case "notify":
                $('#twatter-notification').slideToggle('slow');
...
function addManageKeys(keys) {
    for (let key in keys) {
        $('.manage-keys-house').text(keys[key][0].sharedHouseName);
        let manageHouseKey = `
            <li class="collection-item">
                <div class="row no-padding">
                    <div class="col s9" aria-label="${keys[key][0].sharedName}" data-balloon-pos="down">
                        <span  class="truncate" style="font-weight:bold">${keys[key][0].sharedName}</span>
                    </div>
                    <div class="col s3 right-align">
                        <span class="phone-button manage-keys-remove" data-target-id="${keys[key][0].sharedId}" aria-label="Remove Key" data-balloon-pos="left"><i class="fas red-text fa-user-times fa-2x"></i></span>
                    </div>
                </div>
                <div class="row no-padding">
                    <div class="col s12">
                        <span>Citizen ID: ${keys[key][0].sharedId}</span>
                    </div>
                </div>
            </li>
        `
...
function addAccountInformation(accountInfo) {
    if (accountInfo) {
        ...
        $('.account-information-paycheck').text('$' + (accountInfo.paycheck ? accountInfo.paycheck : 0));
    ...
    licenses = accountInfo.licenses
        ...
        $('.account-information-secondary-job').text(accountInfo.secondaryJob ? accountInfo.secondaryJob : "No Affiliation.");
        }
}
            ...
            if (message && message.handle && message.message) {
                ...
                if (twat !== "") {
                    var twatEntry = $(`<div class="row no-padding">
                                        <div class="col s12">
                                            <div class="card twat-card" style=" border: 2px solid rgba(207, 207, 207, 1.0); background: #19384C;">
                                                <div class="card-content white-text twatter-content">
                                                    <span class="card-title twatter-title">${message.handle}</span>
                                                    <p>${twat}</p>
                                                </div>
                                                <div class="card-action" style="padding-top: 8px;padding-right: 16px; border-top: 2px solid rgba(207, 207, 207, 1.0); padding-bottom: 8px;padding-left: 16px;">
...
$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate'), amount: $(this).data('amount') }));
    setTimeout(function () {
        $.post('http://phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
function openPhoneShell() {
    $(".phone-screen").fadeIn();
    $(".phone-case").slideDown();
    $(".phone-shell").slideDown();
}
