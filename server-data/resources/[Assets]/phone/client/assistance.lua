...
AddEventHandler('phoneGui', function()
  TriggerEvent('jobssystem:current', function(job)
    if job == nil or job == "unemployed" or job == "trucker" or job == exports["isPed"]:isPed("myjob") then
...
AddEventHandler('phone:assistPayJ', function(name)
    ...
    if name == 'taxi' then
      ...
      if driverPed then
        if IsPedInAnyTaxi(ped) then
          TriggerServerEvent("server:givepayJob", "Taxi Service", math.random(20,25))
        else
    ...
    elseif name == 'news' then
      ...
      newsPayment = newsPayment +  math.random(20,25)
      TriggerEvent("DoLongHudText","Payment added to next payout.",2)

   else
      TriggerServerEvent("server:givepayJob", "Assist Blip Payment (Emergency)", math.random(20, 25))
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if nearEntertainment() and exports["srp-base"]:getModule("LocalPlayer"):getVar("job") == "entertainer" then
    ...
    elseif exports["srp-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
      local dist = 0
      if lastBlip.x then
      ...
      if holding and dist < 130 and dist ~= 0 and blipcount >= 1 then
        ...
        TriggerServerEvent("server:givepayJob", "News Reporter", math.random(20,25)) 
        newsPayment = 0
        blipcount = 0 
      ...
      if exports["srp-base"]:getModule("LocalPlayer"):getVar("job") == "entertainer" then
        Citizen.Wait(300000)
      end
    end
      ...
      if (key ~= nil and item ~= nil) then
        ...
        if #(vector2(pos.x, pos.y) - vector2(item.pos.x, item.pos.y)) < 50.0 then
          if item.jobType == "ems" then
            if exports["srp-base"]:getModule("LocalPlayer"):getVar("job") == "ems" then
          ...
          elseif item.jobType == "news" then 
            if exports["srp-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
              TriggerServerEvent('phone:assistRemove', item.id, item.jobType) -- Send message of clear to others
              clearBlip(item)
              if GetTimeDifference(curTime, item.timestamp) > 2000 then
                if canPay then
