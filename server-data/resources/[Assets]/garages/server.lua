	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE repoed = ?', {"0"}, function(vehicles)
		for k, v in ipairs(vehicles) do
			if v.vehicle_state == "Out" then
				exports.ghmattimysql:execute("UPDATE characters_cars SET vehicle_state = @state, current_garage = @garage, coords = @coords WHERE license_plate = @plate", {['garage'] =  v.current_garage, ['state'] = 'In', ['coords'] = nil, ['plate'] = v.license_plate})
			end
		end
	end)
end
...
AddEventHandler('garages:SellToPlayerEnd', function(plate,target,price)
	...
	local userTarg = exports["srp-base"]:getModule("Player"):GetUser(target)
		...
		exports.ghmattimysql:execute("SELECT id FROM characters_cars WHERE license_plate = @plate",{['username'] = player, ['vehicle'] = vehicle, ['plate'] = plate}, function(result)
			if(result[1]) then 
				...
				{['targetCid'] = targetCid, ['plate'] = plate, ["cid"] = character.id, ['phone_number'] = phune_number })
					exports["srp-base"]:AddLog("Vehicle Transfer", user, "Char "..character.id.." Sold a vehicle [" .. plate .. "] to Char" .. targetCid.." for"..price)
