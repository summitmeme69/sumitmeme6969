  ...
  local vehFront = VehicleInFront()
    if vehFront > 0 then
      ...
      if GetEntityModel(vehFront) == GetHashKey(trailer) then
        local finished = exports["srp-taskbar"]:taskBar(25000,"Storing Trailer Contents")
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(1)
    if exports["isPed"]:isPed("myjob") and existingVeh ~= nil then
...
Citizen.CreateThread(function()
  while true do
    Wait(9000)
    if job ~= 0 and exports["isPed"]:isPed("myjob") then
      currentJobPos = returnJobPos(job)
      checkDestruction()
    end
  end
    ...
    if currentJobs[currentJobPos] ~= nil and currentJobs[currentJobPos].JobType == "Shops" then
      ...
      if dist < 20 then
        ...
        if(distance ~= -1 and distance < 5) then
            ...
            TriggerServerEvent("srp-shops:checkOwner",GetPlayerServerId(t),currentJobs[currentJobPos].shopId)
        else
...
AddEventHandler("srp-base:characterLoaded", function()
...
function spawnTruck(i)
      ...
      local finished = exports["srp-taskbar"]:taskBar(3000,"Checking Vehicle")
      if finished == 100 then
      SetVehicleEngineHealth(spawned_car, 1000.0)
      SetVehicleBodyHealth(spawned_car, 1000.0)
      SetVehiclePetrolTankHealth(spawned_car, 1000.0)
