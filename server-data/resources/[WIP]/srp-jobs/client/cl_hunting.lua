...
CanHunt = false
RegisterNetEvent("srp-hunting:allowed")
AddEventHandler("srp-hunting:allowed", function(allowed)
    CanHunt = allowed
end)
...
function StartHuntingSession()
	if allowedHunting == true then
		...
		if OnGoingHuntSession then
			OnGoingHuntSession = false
			if exports["srp-inventory"]:hasEnoughOfItem("100416529",1,false) then
